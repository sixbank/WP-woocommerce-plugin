...
<fieldset id="sixbank-credit-payment-form" class="sixbank-payment-form">
	<ul id="sixbank-card-brand">
		<?php foreach ( $methods as $method_key => $method_name ): ?>
			<li><label title="<?php echo esc_attr( $method_name ); ?>"><i id="sixbank-icon-<?php echo esc_attr( $method_key ); ?>"></i><input type="radio" name="sixbank_credit_card" value="<?php echo esc_attr( $method_key ); ?>" <?php echo ( $first_method == $method_name ) ? 'checked="checked"' : ''; ?>/><span><?php echo esc_attr( $method_name ); ?></span></label></li>
		<?php endforeach ?>
	</ul>
