...
class WC_Sixbank_API {
	...
	/**
	 * Set cURL custom settings for Sixbank.
	 *
	...
	protected function get_default_error_message() {
		$error = new StdClass;
		$error->mensagem = __( 'An error has occurred while processing your payment, please try again or contact us for assistance.', 'sixbank-woocommerce' );
	...
	public function do_transaction( $order, $id, $card_brand, $installments = 0, $credit_card_data = array(), $payment_type = 1) {		
		...
		try{
			...
			$response->message = __($e->getMessage(), 'sixbank-woocommerce');
		}
		return $response;
	}
