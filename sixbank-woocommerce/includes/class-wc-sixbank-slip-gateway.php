	...
	public function order_items_payment_details( $items, $order ) {
		if ( $this->id === $order->payment_method ) {
			...
			if ( 0 < $this->slip_discount ) {
				...
				$items['payment_method']['value'] .= sprintf( __( 'with discount of %s. Order Total: %s.', 'sixbank-woocommerce' ), $this->slip_discount . '%', sanitize_text_field( woocommerce_price( $discount_total ) ) );
			}
